...
tax.change.plot <- final.data %>% group_by(Year) %>% filter(Year<1986, Year> 1970) %>%
  ...
  labs(
    x="Year",
    y = "Share of States",
    title = "Proportion of States with a Change in their Cigarette Tax in Each Year"
  ) + ylim(0,1) +
  theme_bw()
...
high.price.plot <- top.bottom.price %>% filter(change_group=="high") %>%
  ...
  labs(
    x="Year",
    y="Packs per Capita",
    title = "5 States with the Highest Increase in Cigarette Price (Dollars)",
    color= "State"
  ) + theme_bw() +
...
low.price.plot <- top.bottom.price %>% filter(change_group=="low") %>%
  ...
  labs(
    x="Year",
    y="Packs per Capita",
    title = "5 states with the Lowest Increase in Cigarette Prices (Dollars)",
    color= "State"
  ) + theme_bw() +
  scale_x_continuous(breaks=seq(1970,2019,5))
...
graph_5 <- ggplot() +
  ...
  labs(x = "Year", y = "Average Cigarette Packs Sold Per Capita", 
       title = "5 States with Highest Price Increase versus 5 States with Lowest Price Increase",
       color = "Legend") +
...
#Sales from the 5 states with the highest price increase has a steeper decline in average cigarette packs sold per capita compared to the 5 states with the lowest price increase. After 1973 till 2018, the five states with the largest price increase have an average lower sales per capita than the average of the five states’ sales per capita. 
...
ols_part2 <- feols(ln_sales~ln_price_2012, data = final.data %>% filter(Year>=1991))

summary(ols_part2)

#An 1% increase in the cost of a cigarette pack is estimated to decrease sales per capita by 1.07 percent on average. It is an elastic relationship. 
...
#Yes, they are different. The estimates from 1991 to 2015 are more elastic compared to those from 1970 t0 1990. This Amy be because taxes on cigarette packs increased and the Center for Disease Control emphasized the harmful health effects of smoking, making people for sensitive to the price changes on cigarettes.
